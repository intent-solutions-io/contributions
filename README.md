...
| 67 gates across phases A–G | remaining catalog modes are built only after empirical triage |
...
/contribute (SKILL.md) ──spawns──→ subagents: scout · researcher · draft-writer · test-runner
        ...
        ▼
   transition.sh ──→ gate-runner.sh ──→ 67 gates (phases A–G)   ──BLOCK/WARN/PASS (logged)
        │
        ▼
